...
// Create a bucket with public read access
	...
	// Put our file (also with public read access)
		...
		//$array= print_r($contents, 1);
		//print_r($contents, 1);
		//print_r(json_encode($contents, 1));


		// Get object info
		//$info = $s3->getObjectInfo($bucketName, baseName('/share/TReport.docx'));
		//echo "S3::getObjectInfo(): Info for {$bucketName}/".baseName('share/TReport.docx').': '.print_r($info, 1);

		$bucketName = 'musixcloud';
	  //$a=$s3->getObject($bucketName, 'share/email_art.png');
		//return $a;
		//print_r(json_encode($info));

		// You can also fetch the object into memory
		$saveName="email_art.png";
		//header('Content-Type: application/octet-stream');
		//$reslt=var_dump($s3->getObject($bucketName,'share/email_art.png'));
		//header("Content-Type: {$result['headers']['type']}");
		$result = $s3->getAuthenticatedURL($bucketName, 'share/email_art.png', 60);
		print_r("<a href='https://s3-ap-southeast-1.amazonaws.com/musixcloud/share/email_art.png'>https://s3-ap-southeast-1.amazonaws.com/musixcloud/share/email_art.png</a>");
